...
Author URI: http://criacaocriativa.com.br/plugins/
					...
					if ( post_type_exists( sanitize_text_field($wp_slug_language[$i]['post_type']) ) ) {
						...
						if(get_locale() == sanitize_text_field($wp_slug_language[$i]['languages'])) {
							$args = array(
								...
								'public'					=> true,
								'labels' => array (
									'name'					=> sanitize_text_field($get_post_type_object->labels->name),
									'singular_name'			=> sanitize_text_field($get_post_type_object->labels->singular_name),
									'menu_name'				=> sanitize_text_field($get_post_type_object->labels->menu_name),
									'add_new'				=> sanitize_text_field($get_post_type_object->labels->add_new),
									'add_new_item'			=> sanitize_text_field($get_post_type_object->labels->add_new_item),
									'edit'					=> sanitize_text_field($get_post_type_object->labels->edit),
									'edit_item'				=> sanitize_text_field($get_post_type_object->labels->edit_item),
									'new_item'				=> sanitize_text_field($get_post_type_object->labels->new_item),
									'view'					=> sanitize_text_field($get_post_type_object->labels->view),
									'view_item'				=> sanitize_text_field($get_post_type_object->labels->view_item),
									'search_items'			=> sanitize_text_field($get_post_type_object->labels->search_items),
									'not_found'				=> sanitize_text_field($get_post_type_object->labels->not_found),
									'not_found_in_trash'	=> sanitize_text_field($get_post_type_object->labels->not_found_in_trash)
								),
								'rewrite' => array(
									'slug'       => sanitize_text_field($wp_slug_language[$i]['slug_post_type']),
									'with_front' => true
								),
							);				
							register_post_type( sanitize_text_field($wp_slug_language[$i]['post_type']), $args );
						}
					}
				}
